...
async function ensureIndexes(db) {
  ...
  await Promise.all([
    ...
    db.collection('emailSends').createIndex({ token: 1 }, { unique: true }),
    db.collection('emailSends').createIndex({ email: 1, sendIndex: 1 }),
    db.collection('emailSends').createIndex({ waitlistId: 1, sentAt: -1 })
  ]);
  indexesReady = true;
}
